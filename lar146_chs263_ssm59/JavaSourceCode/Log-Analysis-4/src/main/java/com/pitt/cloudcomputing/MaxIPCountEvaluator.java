...
public class MaxIPCountEvaluator {

	private final static char SPACE = ' ';
	 
	private static Tuple2<String, Integer> apply(String s) {
		 int end = s.indexOf(SPACE);
		 String ip = s.substring(0, end).trim();
		 int count = 1;
		 return new Tuple2<String, Integer>(ip, count);
	}
	...
	private static void analysis(String[] args) {
		...
		SparkConf conf = new SparkConf().setAppName("Log Analysis 4");
